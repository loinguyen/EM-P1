...
import vn.com.lco.storm.tools.NthLastModifiedTimeTracker;
import vn.com.lco.storm.tools.SlidingWindowCounter;
import vn.com.lco.storm.util.TupleHelpers;
...
public class RollingCountBolt extends BaseRichBolt {
    ...
    private void emit(Map<Object, Long> counts, int actualWindowLengthInSeconds) {
        for (Entry<Object, Long> entry : counts.entrySet()) {
            ...
            Log.info("RollingCountBolt - emit");
            Log.info("obj: " + obj + " | count: " + count + " | actualWindowLengthInSeconds: " + actualWindowLengthInSeconds);
	...
	/**
	 * edit by OaiDQ.
	 * Get eventId and operation. From operation, we'll calculate point for event.
	 * Pass eventId and point to incrementCount method of base class.
	 */
    private void countObjAndAck(Tuple tuple) {
        ...
        int point = 0;
        if (operation.equals(Constants.OPERATON_VIEW)) {
        	point = 1;
        } else if (operation.equals(Constants.OPERATON_LIKE)) {
        	point = 2;
        ...
        Log.info("RollingCountBolt - countObjAndAck");
        Log.info("eventId & point: " + event + " - " + point);
        counter.incrementCount(event, point);
        collector.ack(tuple);
    }
