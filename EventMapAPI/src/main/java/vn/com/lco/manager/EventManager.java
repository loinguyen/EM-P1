...
public class EventManager {
	...
	public Event saveEvent(EventDTO eventDTO) {
		...
		//find and Update number of created event of creator User
		User creator = userRepository.findById(eventDTO.getUserId());
		creator.setNoOfCreatedEvents(creator.getNoOfCreatedEvents() + 1);
		...
		if (notifications != null) {
			...
			while (iterator.hasNext()) {
				...
				if (userUpdateList != null) {
					...
					userUpdateList = new LinkedList<String>();
					userUpdateList.add(event.getUserId());
